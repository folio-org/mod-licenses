...
| `KAFKA_HOST`                    | Hostname of the Kafka broker used to publish License domain events (CREATE/UPDATE). Publishing is best-effort: an unreachable broker neither stops the module nor fails REST writes — failures are logged and swallowed.                                                            | Hostname                                                                                                                                             | `localhost` |
| `KAFKA_PORT`                    | Port of the Kafka broker.                                                                                                                                                                                                                                                             | Integer                                                                                                                                              | `9092`  |
| `ENV`                           | Logical deployment name used as the Kafka topic-name prefix (must be unique across all environments sharing the same Kafka cluster).                                                                                                                                                 | String                                                                                                                                               | `folio` |
| `KAFKA_TENANT_COLLECTION`       | Tenant partitioning mode for Kafka topics. `ALL` (default, matches folio-kafka-wrapper) publishes every tenant's events to a single topic per entity (e.g. `folio.ALL.licenses.license`); consumers route by the `tenant` field in the event envelope. Any other value falls back to per-tenant topics (e.g. `folio.diku.licenses.license`). | String                                                                                                                                               | `ALL`   |
